...
{
    // Specify a callback to be executed when using this method.
...
{
    // The function returns a future object, and not an actual object.
    ...
    auto limitations_async = limitations_future_async.get();  //use get() to access the actual object
...
{
    // RouterClientSendOptions exists to modify the default behavior of
    // of the router (block method). Router default values are 
    //     andForget = False
    //     delay_ms = 0
    //     timeout_ms = 10000
    ...
    router_options.timeout_ms = 5000;  // (milliseconds)  //the timeout for a block method function to wait
